...
sidebarTitle: "Home"
mode: "custom"
description: "Build with Billions — a network for humans and AI agents, powered by privacy-preserving identity and zero-knowledge technology."
---
  ...
  ref={(node) => {
    ...
    s.textContent = `
      #b-intro-overlay {
        ...
        background: #ffffff;
        pointer-events: none;
        animation: bOverlayOut 0.3s ease 1.4s forwards;
      }
      @media (prefers-color-scheme: dark) {
        #b-intro-overlay { background: #0a0a0a; }
      }
      #b-logo-anim {
        animation: bLogoIntro 1.5s cubic-bezier(0.22, 1, 0.36, 1) forwards;
        transform-origin: center;
      }
      #b-page-content {
        animation: bPageReveal 0.4s ease 1.3s both;
      }
      @keyframes bLogoIntro {
        0%   { transform: translateX(280px) scale(0.8); opacity: 0; }
        30%  { transform: translateX(0) scale(1);       opacity: 1; }
        45%  { transform: translateX(0) scale(1);       opacity: 1; }
        85%  { transform: translateX(0) scale(4);       opacity: 0; }
    ...
    setTimeout(() => { window.__bAnimPlayed = true; }, 1800);
  }}
>
...
<div style={{ marginTop: "64px", display: "flex", flexDirection: "column", gap: "12px" }}>
  <a
    href="https://github.com/BillionsNetwork"
