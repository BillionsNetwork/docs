...
Billions Network issues [Proof-of-Uniqueness](/billions-wallet/proof-of-uniqueness) attestations that cryptographically bind an agent to a verified human. The attestation is an on-chain record with a schema ID and a **nullifier** — a privacy-preserving identifier that uniquely represents the human without revealing their identity.
